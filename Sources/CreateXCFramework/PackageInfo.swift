...
//    let package: Package
    ...
    init (options: Command.Options) throws {
        ...
        #if swift(>=5.5)
            resources = try UserManifestResources(swiftCompiler: self.toolchain.swiftCompiler, swiftCompilerFlags: [])
        #else
            resources = try UserManifestResources(swiftCompiler: self.toolchain.swiftCompiler)
        #endif
